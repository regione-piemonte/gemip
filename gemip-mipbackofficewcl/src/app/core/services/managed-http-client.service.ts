...
export class ManagedHttpClient {
  ...
  get<E>(url: string, options?: { params: HttpParams, responseType?: 'json'; }): Observable<E> {
    return this.handle(this.http.get<E>(url, options));
  }
  getResponseType<E>(url: string, params: HttpParams, responseType?: string): Observable<E> {
    return this.handle(this.http.get<E>(url, { params, responseType: responseType as 'json' }));
  }
  post<E>(url: string, body: any, params?: HttpParams, responseType?: string, headers?: HttpHeaders): Observable<E> {
