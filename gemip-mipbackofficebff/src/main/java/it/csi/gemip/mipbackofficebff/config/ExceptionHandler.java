package it.csi.gemip.mipbackofficebff.config;
...
import it.csi.gemip.mipbackofficebff.exception.*;

import it.csi.gemip.mipbackofficebff.helper.ErrorHelper;
...
public class ExceptionHandler implements ExceptionMapper<RuntimeException> {
	...
	private Response handleUtenteNonAbilitatoException(UtenteNonAbilitatoException exception) {
		return Response.status(Status.FORBIDDEN).entity(ErrorHelper.createErrore(Status.FORBIDDEN.getStatusCode(), exception.getMessage())).build();
	}
