...
import it.csi.gemip.mipcittadinobff.bff.CittadiniApi;
...
public class CittadiniApiImpl extends ParentApiImpl implements CittadiniApi{
    ...
    @ConfigProperty(name="shibboletDisable",defaultValue = "false")
    ...
    public Response getDashboardCittadino(Cittadino body, SecurityContext securityContext, HttpHeaders httpHeaders, HttpServletRequest httpRequest) {
        ...
        //per tester
        UserInfo userInfo = (UserInfo) httpRequest.getAttribute(USERINFO_SESSIONATTR);

        if(userInfo != null && !shibboletDisable.get()==true) {
            body.setCodiceFiscale(userInfo.getCodFisc());
            body.setNome(userInfo.getNome());
            body.setCognome(userInfo.getCognome());
        }
