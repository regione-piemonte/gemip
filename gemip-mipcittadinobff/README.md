# Prodotto - Componente
Prodotto GEMIP, componente MIPCITTADINOBFF

# Descrizione della componente
Componente Cloud Native che opera come BFF (Backend For Frontend) per l'applicativo _partecipa_. \
...
Avvio progetto:
```
docker run -v ${PWD}:/usr/src/mymaven -v ${PWD}/.m2:/root/.m2 -p 8081:8081 -it -w /usr/src/mymaven maven:3.9.2-eclipse-temurin-17 mvn clean compile 
```
Esecuzione:

Lanciando il comando docker che utilizza una cartella .m2 di progetto
```
docker run -v ${PWD}:/usr/src/mymaven -v ${PWD}/.m2:/root/.m2 -p 8081:8081 -p 5005:5005 -it -w /usr/src/mymaven maven:3.9.2-eclipse-temurin-17 mvn quarkus:dev "-DdebugHost=0.0.0.0" "-Ddebug=5005" "-Dquarkus.http.host=0.0.0.0"
```
Lanciando il comando docker che utilizza la cartella .m2 globale
```
docker run -v ${PWD}:/usr/src/mymaven -v ${HOME}/.m2:/root/.m2 -p 8081:8081 -p 5005:5005 -it -w /usr/src/mymaven maven:3.9.2-eclipse-temurin-17 mvn quarkus:dev "-DdebugHost=0.0.0.0" "-Ddebug=5005" "-Dquarkus.http.host=0.0.0.0"
```

Se non è stato installato e reso raggiungibile il db, l'applicativo allo startup andra in eccezione ma sarà comunque possibile verificare che, per il resto, il dispiegamento sia stato effettuato correttamente puntando, per esempio a `http://localhost:8081/api/status`.

Esempio di output nel caso di problemi allo startup invocando http://localhost:8081/api/status:
...
```
docker run -v ${PWD}:/usr/src/mymaven -v ${PWD}/.m2:/root/.m2 -p 8081:8081 -p 5005:5005 -it --network host -w /usr/src/mymaven maven:3.9.2-eclipse-temurin-17 mvn quarkus:dev "-DdebugHost=0.0.0.0" "-Ddebug=5005" "-Dquarkus.http.host=0.0.0.0"
```
...
```
docker run -v ${PWD}:/usr/src/mymaven -v ${PWD}/.m2:/root/.m2 -p 8081:8081 -p 5005:5005 -it --network host -w /usr/src/mymaven maven:3.9.2-eclipse-temurin-17 mvn quarkus:dev "-DdebugHost=0.0.0.0" "-Ddebug=5005" "-Dquarkus.http.host=0.0.0.0" "-Dquarkus.http.port=8081"
```

potrà comunicare con la componente MIPCOMMONAPI in ascolto sulla porta 8080, invocando per esempio:
`localhost:8081/api/cittadinanze`
